...
The above also holds for plotting functions, however, in this case it is required that the plots generate baseline and test images. The following documents the current best practice. See `test_plot.py <https://github.com/pyfar/pyfar/blob/main/tests/test_plot.py>`_ for a code example.
...
Test images show how an image produced by a given plot command currently looks like. Test images must be created each time the tests run. If the behavior of a pot function changes, the corresponding test image will change. In this case, the changed test image must be visually compared to the corresponding baseline image as part of a pull request.
