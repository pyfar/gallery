...
The above also holds for plot functions, however, in this case it is required that the tests generate baseline and test images. The following documents the current best practice. See `test_plot.py <https://github.com/pyfar/pyfar/blob/main/tests/test_plot.py>`_ for a code example.
